...
@php
    ...
    $items = $items ?? [
        ['id'=>1,'nama'=>'Kemeja Flanel Kotak Vintage','kategori'=>'Kemeja','ukuran'=>'M','warna'=>'Hitam/Merah','kondisi'=>'Sangat Baik','harga'=>45000,'stok'=>1,'foto'=>null,'tag'=>['Casual','Vintage'],'cocok'=>false],
        ['id'=>2,'nama'=>'Kaos Oversize Polos Putih','kategori'=>'Kaos','ukuran'=>'M','warna'=>'Putih','kondisi'=>'Baik','harga'=>30000,'stok'=>1,'foto'=>null,'tag'=>['Casual','Minimalis'],'cocok'=>false],
        ['id'=>3,'nama'=>'Cardigan Rajut Krem','kategori'=>'Cardigan','ukuran'=>'M','warna'=>'Krem','kondisi'=>'Sangat Baik','harga'=>60000,'stok'=>1,'foto'=>null,'tag'=>['Minimalis','Casual'],'cocok'=>false],
        ['id'=>4,'nama'=>'Celana Jeans Slim Biru','kategori'=>'Celana Jeans','ukuran'=>'M','warna'=>'Biru','kondisi'=>'Baik','harga'=>55000,'stok'=>1,'foto'=>null,'tag'=>['Casual'],'cocok'=>false],
        ['id'=>5,'nama'=>'Jaket Bomber Hitam','kategori'=>'Jaket','ukuran'=>'L','warna'=>'Hitam','kondisi'=>'Baik','harga'=>75000,'stok'=>1,'foto'=>null,'tag'=>['Streetwear'],'cocok'=>false],
        ['id'=>6,'nama'=>'Dress Floral Pendek','kategori'=>'Dress','ukuran'=>'S','warna'=>'Multicolor','kondisi'=>'Sangat Baik','harga'=>50000,'stok'=>1,'foto'=>null,'tag'=>['Feminine'],'cocok'=>false],
        ['id'=>7,'nama'=>'Kemeja Linen Putih','kategori'=>'Kemeja','ukuran'=>'M','warna'=>'Putih','kondisi'=>'Sangat Baik','harga'=>48000,'stok'=>1,'foto'=>null,'tag'=>['Casual','Minimalis'],'cocok'=>false],
        ['id'=>8,'nama'=>'Celana Kargo Olive','kategori'=>'Celana','ukuran'=>'M','warna'=>'Olive','kondisi'=>'Baik','harga'=>52000,'stok'=>1,'foto'=>null,'tag'=>['Streetwear','Casual'],'cocok'=>false],
    ];
...
<form id="form-pilihan" action="{{ url('/kurator/edit-pilih-item/' . $box->id) }}" method="POST" class="hidden">
    @csrf
...
<div class="fade-in space-y-6">
    ...
    <div class="flex gap-6 items-start">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div id="grid-items" class="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div class="item-card card-wood rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-md cursor-pointer"
                     data-id="{{ $item['id'] }}"
                     data-nama="{{ $item['nama'] }}"
    ...
    <div id="float-bar"
        ...
        <button type="button" id="float-btn-lanjut" onclick="lanjutSusunBox()"
...
<style>
    .item-card.selected { outline: 2px solid #C85A1A; outline-offset: 2px; }
    .item-card.selected .check-overlay { background: #C85A1A; border-color: #C85A1A; }
    .item-card.selected .check-icon { display: inline !important; color: white; }
    .item-card.over-limit { opacity: 0.45; pointer-events: none; }
    .item-card[data-cocok="0"] { opacity: 0.75; }
</style>
...
<script>
    ...
    // hapus jika perlu
    // Restore pilihan sebelumnya dari session
    const pilihanSebelumnya = @json($pilihanSebelumnya ?? []).map(String);
    if (pilihanSebelumnya.length > 0) {
        document.querySelectorAll('.item-card').forEach(function(card) {
            if (pilihanSebelumnya.includes(card.dataset.id)) {
    ...
    function toggleItem(card) {
        ...
        if (selected[id]) {
            ...
            selected[id] = { nama: nama, harga: harga };
            card.classList.add('selected');
        }
        updateUI();
    }
    ...
    function updateUI() {
        ...
        ['btn-lanjut', 'btn-lanjut-bottom', 'float-btn-lanjut'].forEach(function(btnId) {
        ...
        document.querySelectorAll('.item-card').forEach(function(card) {
            if (count >= MAX_ITEM && !selected[card.dataset.id]) {
    ...
    function lanjutSusunBox() {
        if (Object.keys(selected).length === 0) return;

        var container = document.getElementById('hidden-item-inputs');
        container.innerHTML = '';
        ...
        document.getElementById('form-pilihan').submit();
    }

    function applyFilter() {
        var search    = document.getElementById('input-search').value.toLowerCase();
        var filterKat = document.getElementById('filter-kategori').value;
        var filterUk  = document.getElementById('filter-ukuran').value;
        var onlyCocok = document.getElementById('toggle-cocok').checked;

        var visible = 0;
        document.querySelectorAll('.item-card').forEach(function(card) {
            var show =
                card.dataset.nama.toLowerCase().includes(search) &&
                (!filterKat || card.dataset.kategori === filterKat) &&
                (!filterUk  || card.dataset.ukuran   === filterUk) &&
                (!onlyCocok || card.dataset.cocok     === '1');
