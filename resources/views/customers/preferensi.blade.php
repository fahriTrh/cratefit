...
<div class="fade-in">
    ...
    {{-- PAGE HEADER --}}
    <div class="mb-8">
        <p class="mb-1 font-display">Langkah 2 dari 4</p>
        <h1 class="font-display text-3xl text-crate-text font-bold">Preferensi Fashion Kamu</h1>
        <p class="text-gray-500 font-body mt-1 text-sm">Ceritakan gaya kamu — kurator kami akan menyusun box yang pas banget!</p>
    </div>
    ...
    <form action="" method="POST" class="space-y-6">
        ...
        <div class="card-wood rounded-2xl p-6" style="animation-delay:0.05s">
            <h2 class="font-display text-lg text-crate-text font-bold mb-1 flex items-center gap-2">
                <i data-lucide="ruler" class="w-5 h-5 text-crate-primary"></i> Ukuran Pakaian
            </h2>
            <p class="text-gray-500 text-xs font-body mb-5">Pilih ukuran yang biasa kamu pakai</p>

            <div class="grid grid-cols-2 sm:grid-cols-3 gap-5">

                {{-- Atasan --}}
                <div>
                    <label class="block text-xs font-body font-semibold text-crate-text/70 uppercase tracking-wider mb-2">Atasan</label>
                    <div class="flex flex-wrap gap-2">
                        ...
                        <label class="cursor-pointer">
                            ...
                            <span class="tag-btn block px-3 py-1.5 rounded-lg border border-crate-accent bg-white
                ...
                {{-- Bawahan --}}
                <div>
                    <label class="block text-xs font-body font-semibold text-crate-text/70 uppercase tracking-wider mb-2">Bawahan</label>
                    <div class="flex flex-wrap gap-2">
                        @foreach(['XS','S','M','L','XL','XXL'] as $s)
                        <label class="cursor-pointer">
                ...
                <div class="sm:col-span-1 space-y-3">
                    <div>
                        ...
                        <input type="number" name="tinggi_badan" value="{{ old('tinggi_badan', $preferensi->tinggi_badan ?? '') }}"
                            placeholder="cth: 165"
                            class="w-full border border-crate-accent rounded-xl px-4 py-2.5 text-sm font-body text-crate-text
                                      bg-white placeholder-crate-stone transition-all">
        ...
        <div class="card-wood rounded-2xl p-6" style="animation-delay:0.1s">
            <h2 class="font-display text-lg text-crate-text font-bold mb-1 flex items-center gap-2">
                <i data-lucide="palette" class="w-5 h-5 text-crate-primary"></i> Gaya Berpakaian
            </h2>
            <p class="text-gray-500 text-xs font-body mb-5">Pilih satu atau lebih gaya favoritmu</p>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <label class="cursor-pointer group">
                    ...
                    <div class="tag-btn h-full border-2 border-crate-accent bg-white rounded-2xl p-4 text-center
                                    peer-checked:border-crate-primary peer-checked:bg-crate-primary/5
                                    hover:border-crate-primary transition-all">
                        <div class="mb-1 flex justify-center">
                            <i data-lucide="{{ $style['icon'] }}" class="w-7 h-7 text-crate-primary"></i>
                        </div>
                        <p class="font-body font-semibold text-crate-text text-sm">{{ $style['name'] }}</p>
                        <p class="font-body text-gray-500 text-xs mt-0.5">{{ $style['desc'] }}</p>
                    </div>
                </label>
                @endforeach
            </div>
        ...
        <div class="card-wood rounded-2xl p-6" style="animation-delay:0.15s">
            <h2 class="font-display text-lg text-crate-text font-bold mb-1 flex items-center gap-2">
                <i data-lucide="palette" class="w-5 h-5 text-crate-primary"></i> Warna Favorit
            </h2>
            <p class="text-gray-500 text-xs font-body mb-5">Pilih warna yang sering kamu pakai (maks. 5)</p>
            ...
            <div class="flex flex-wrap gap-4">
                ...
                <label class="cursor-pointer flex flex-col items-center gap-1.5" title="{{ $color['name'] }}">
                    ...
                    <div class="swatch-btn w-9 h-9 rounded-full border-2 border-transparent peer-checked:border-crate-primary
                                    shadow-sm transition-all ring-1 ring-black/10"
                        style="background:{{ $color['hex'] }}"></div>
                    <span class="text-crate-text/60 text-xs font-body text-center leading-tight" style="max-width:48px">{{ $color['name'] }}</span>
                </label>
                @endforeach
            </div>
        ...
        <div class="card-wood rounded-2xl p-6" style="animation-delay:0.2s">
            <h2 class="font-display text-lg text-crate-text font-bold mb-1 flex items-center gap-2">
                <i data-lucide="shirt" class="w-5 h-5 text-crate-primary"></i> Jenis Pakaian yang Diinginkan
            </h2>
            <p class="text-gray-500 text-xs font-body mb-5">Pilih item apa yang boleh masuk ke dalam box kamu</p>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <label class="cursor-pointer">
                    ...
                    <span class="tag-btn inline-block px-4 py-2 rounded-full border border-crate-accent bg-white
                                     text-sm font-body text-crate-text
                                     peer-checked:bg-crate-primary peer-checked:text-white peer-checked:border-crate-primary
                                     hover:border-crate-primary transition-all">
                        {{ $item }}
        ...
        <div class="card-wood rounded-2xl p-6" style="animation-delay:0.25s">
            <h2 class="font-display text-lg text-crate-text font-bold mb-1 flex items-center gap-2">
                <i data-lucide="ban" class="w-5 h-5 text-crate-primary"></i> Pantangan & Catatan Khusus
            </h2>
            <p class="text-gray-500 text-xs font-body mb-5">Ceritakan item yang TIDAK ingin kamu terima, atau hal lain yang perlu diketahui kurator</p>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-wrap gap-2 mb-3">
                        ...
                        <label class="cursor-pointer">
                            <input type="checkbox" name="pantangan[]" value="{{ $pantang }}" class="sr-only peer"
                                ...
                                <span class="tag-btn inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-crate-accent bg-white
                                                text-sm font-body text-crate-text
                ...
                <div>
                    ...
                    <textarea name="catatan_kurator" rows="4"
                        ...
                        class="w-full border border-crate-accent rounded-xl px-4 py-2.5 text-sm font-body text-crate-text
bg-white placeholder-gray-400 transition-all">{{ old('catatan_kurator', $preferensi->catatan_kurator ?? '') }}</textarea>
